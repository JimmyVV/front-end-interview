...
1. [ translate(X,Y) 是如何对应于矩阵变换的？][13]

等同于 `matrix(1, 0, 0, 1, X, Y); ` 

2. [ matrix 属性值一共有几个？分别代表什么含义？][14]

`matrix(a,b,c,d,e,f);` 等同于 ![此处输入图片的描述][15]

3. bezier 曲线有了解么？其 4 个点分别有什么含义？

P0,P1 在一条直线上，P2,P3在一条直线上。其中，P2、P3 x,y 必须在 (0,1) 范围内。而 P0 为 (0,0)，P3 为 (1,1)

![image.png-70.1kB][16]

4. 在 keyframe-Animation 中，我们常常使用 bezier 曲线做什么用？
...
1. [使用 Webpack 来导出一个 `demo.js` 的库，规定可以通过全局变量、require\import 等方式使用，应该设置哪些属性？][53]
...
2. [如何修改通过 `import` 或者 `require` 引用的解析路径？][54]

通过设置 `resolve` 属性值中的 `alias`、`modules` 来完成。


3. webpack plugin 和 loader 有什么区别吗？loader、plugin  常用来处理什么？
